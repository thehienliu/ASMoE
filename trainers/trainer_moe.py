...
class EVJVQATrainer:
  ...
  def train_epoch(self, train_dataloader: DataLoader):
    ...
    for batch in tqdm(train_dataloader):
        ...
        aux_loss.append(batch_metrics['aux_loss'])
        total_loss.append(batch_metrics['total_loss'])
    ...
    scalar_metrics = {
        ...
        'Loss': mean(loss),
        'Auxality-Loss': mean(aux_loss),
        'Total-Loss': mean(total_loss),
        'F1': mean(f1),
  ...
  def train_step(self, batch):
    ...
    if self.mixed_precision:
      with torch.autocast(device_type="cuda", dtype=torch.float16):
        ...
        # Forward pass
        outputs = self.model(batch) # (predictions (`torch.Tensor` with shape `batch_size, seq_len, vocab_size`) , aux_loss (`torch.Tensor` a float values))

        # Calculate loss
        loss = self.criterion(outputs[0].mT, batch.answers_tokenized.input_ids)
        total_loss = loss + outputs[1]
    ...
    batch_metrics['aux_loss'] = outputs[1].item()
